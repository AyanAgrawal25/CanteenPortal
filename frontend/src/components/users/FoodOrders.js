...
init("user_TsZ47NcJxTLbs0FsGLimv");
...
const UsersList = (props) => {
  ...
  const accept = (event) => {
    ...
    console.log(user1);
    axios
      .post("http://localhost:4000/user/incct", user1)
      .then((response) => {
        console.log(response.data);
        localStorage.setItem("count",response.data.count);
        // window.location = "/foodorders";
      })
      .catch((err) => console.log(err));
    const newItem = {
      id: localStorage.getItem("id"),
    ...
    console.log(newItem);
    axios
      .post("http://localhost:4000/buy/accFood", newItem)
      .then((response) => {
        console.log(response.data);
        // window.location = "/foodorders";
        emailjs
          .send("service_kfros5e", "template_s8poc28", {
            to_name: "Sir",
            from_name: localStorage.getItem("name"),
            message: "Your order has been accepted!",
          })
          .then((good) => {
            if (good) window.location = "/foodorders";
          });
      })
      .catch((err) => console.log(err));
  };
  ...
  const cooking = (event) => {
    ...
    console.log(newItem);
    axios
      .post("http://localhost:4000/buy/cookFood", newItem)
      .then((response) => {
        console.log(response.data);
        window.location = "/foodorders";
      })
  ...
  const reject = (event) => {
    ...
    const user1 = {
      ...
      amt: localStorage.getItem("ret_amt"),
    };
    console.log(user1);
    axios
      .post("http://localhost:4000/user/updwallet", user1)
      .then((response) => {
        console.log(response.data);
        // window.location = "/foodorders";
      })
    ...
    console.log(newItem);
    axios
      .post("http://localhost:4000/buy/rejFood", newItem)
      .then((response) => {
        console.log(response.data);
        // window.location = "/foodorders";
        emailjs
          .send("service_kfros5e", "template_8r33p98", {
            to_name: "Sir",
            from_name: localStorage.getItem("name"),
            message:
              "Apologies for inconvenience, Your order has been rejected! We hope to see you again!",
          })
          .then((good) => {
            if (good) window.location = "/foodorders";
          });
  ...
  const pickup = (event) => {
    ...
    console.log(user1);
    axios
      .post("http://localhost:4000/user/decct", user1)
      .then((response) => {
        console.log(response.data);
        localStorage.setItem("count",response.data.count);
        // window.location = "/foodorders";
    ...
    console.log(newItem);
    axios
      .post("http://localhost:4000/buy/pickupFood", newItem)
      .then((response) => {
        console.log(response.data);
        window.location = "/foodorders";
      })
                            ...
                            <Button
                              ...
                              onClick={() => {
                                ...
                                if(Number(localStorage.getItem("count"))<10)
                                 accept();
                                else alert("Can't accept more orders")
                              }}
                            >
                              Move to next stage
                            </Button>
                        ...
                        <TableCell>
                          {user.status === "Placed" && (
                            <Button
                              ...
                              onClick={() => {
                                localStorage.setItem("id", user._id);
                                localStorage.setItem("buyer_email", user.buyer);
                                localStorage.setItem("ret_amt", user.cost);
                                reject();
                              }}
                            >
                              Reject
